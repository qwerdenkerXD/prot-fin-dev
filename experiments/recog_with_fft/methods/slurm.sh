...
for i in \"\${overlaps[@]}\"; do
    name=WINSIZE_$3_NPEAKS_$4_OVERLAP_\${i}
    WINDOW_SIZE=$3 OVERLAP=\${i} N_PEAKS=$4 python3 protfin.py create-db -p ../results/${exp}/\$name.pickle $1
    WINDOW_SIZE=$3 OVERLAP=\${i} N_PEAKS=$4 python3 protfin.py find-matches -d ../results/${exp}/\$name.pickle ../results/${exp}/_test_selection.fa > ../results/${exp}/_\${name}_test_selection.matches
