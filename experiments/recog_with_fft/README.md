...
|[incl_ampl_1bit.summary.csv](./results/incl_ampl_1bit.summary.csv)|a summary of the found matches for 217 protein sequences with including the amplitudes in 1 Bit of a hash 
|[incl_ampl_2bit.summary.csv](./results/incl_ampl_2bit.summary.csv)|a summary of the found matches for 217 protein sequences with including the amplitudes in 2 Bits of a hash.
|[summary.csv](./results/summary.csv)|a summary of the match finding results. The 2-Bit-variant seems to be the best, as median and average first match counts are close to 1.
...
[incl_ampl_1bit.summary.csv](./results/incl_ampl_1bit.summary.csv):
```sh
cd methods
materials=../../../materials
BITS=1 python3 protfin.py create-db $materials/protein.fa -p incl_ampl_1bit.pickle
python3 evaluation.py select-samples $materials/mapmanreferencebins.results.txt $materials/protein.fa -s 7 > ../results/_test_selection.fa
BITS=1 python3 protfin.py find-matches ../results/_test_selection.fa -d incl_ampl_1bit.pickle > ../results/_incl_ampl_1bit.matches
python3 evaluation.py eval ../results/_incl_ampl_1bit.matches > ../results/incl_ampl_1bit.summary.csv
```

[incl_ampl_2bit.summary.csv](./results/incl_ampl_2bit.summary.csv):
```sh
cd methods
materials=../../../materials
BITS=2 python3 protfin.py create-db $materials/protein.fa -p incl_ampl_2bit.pickle
python3 evaluation.py select-samples $materials/mapmanreferencebins.results.txt $materials/protein.fa -s 7 > ../results/_test_selection.fa
BITS=2 python3 protfin.py find-matches ../results/_test_selection.fa -d incl_ampl_2bit.pickle > ../results/_incl_ampl_2bit.matches
python3 evaluation.py eval ../results/_incl_ampl_2bit.matches > ../results/incl_ampl_2bit.summary.csv
```
...
BITS=2 python3 protfin.py create-db $materials/protein.fa -p incl_ampl_2bit.pickle
TITLE="Distribution of sequences' hash counts" X_LABEL="Hash counts" \
Rscript raincloud_plot.R "No Amplitude" <(py evaluation.py print-hash-counts database.pickle) "2 Bit Amplitude" <(py evaluation.py print-hash-counts incl_ampl_2bit.pickle) "1 Bit Amplitude" <(py evaluation.py print-hash-counts incl_ampl_1bit.pickle) ../results/hash_count_dist.png
```

---
## Discussion/Brainstorming
As of the summary, the recognition is way more accurate if including the STFT amplitudes into the hashes. Currently, only the matches with first rank were analyzed, to check if the original match is the best.<br>
Next thing to do is to look at the following ranks on familiar mapman bins.

Some ideas for future development:
 - add appropriate testing
 - check more than first rank only on familiar mapman bins (`extend_protfin_out.awk`)
