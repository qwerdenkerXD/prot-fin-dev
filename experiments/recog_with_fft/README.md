...
## Results (`results/*`)
```diff
- WARNING: Some parameter constellations may be missing in results. This is because of to high memory usage or computation time.
```
...
python3 summary.py ../results/_v0.3_stft_param_exp/*.summary.csv > ../results/summary.csv
```
...
As of the results, the F1 Score is below 0.1 for all tried combinations of parameters. This may be caused by the fact that there are that few frequencies per window (max. 25 for a window size 50), so the probability of shared hashes between unrelated proteins is potentially high.

To get around this, it may be necessary to do a FFT on the full sequence or invent a very good peak selection method.

Some ideas for future development:
 - add appropriate testing
 - recognition with FFT on full sequence instead of STFT
 - analyze amplitudes of frequencies to get a range of potentially significant amplitudes
