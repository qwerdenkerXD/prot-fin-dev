# Recognition with Fast Fourier Transformation <br> v0.2-exp-hashed_amplitudes
`NOTE: This README and the code are updated synchronously.`

This experiment is to analyze the influence of the amplitudes from STFT on hashes if included. Currently, only frequencies form the constellation map and the hashes are like edges between the dots.<br>
With including the amplitudes, these hashes will be like vectors in a 3-dim constellation map. So they represent much more information.<br>
To avoid overfitting, the amplitudes will be included as comparison results of which of the both frequencies has a higher or lower amplitude.
...
## Results (`results/*`)
```diff
- WARNING: Some parameter constellations may be missing in results. This is because of too high memory usage or computation time.
```

|                          file                            |     content
|----------------------------------------------------------|------------------
|[runtimes_createdb.csv](./results/runtimes_createdb.csv)  |Overview of the calculation times for database creation, including the resulting database's size
|[summary.csv](./results/summary.csv)                      |Summary of all parameters' results
...
python3 summary.py ../results/_v0.3-exp-hashed_amplitudes/*.summary.csv > ../results/summary.csv
```

[runtimes_createdb.csv](./results/runtimes_createdb.csv):
```sh
cd methods
materials=../../../materials
bash slurm.sh $materials/protein.fa $materials/mapmanreferencebins.results.txt
...
Comparing the results with the recognition without the amplitudes, as done in [v0.3-exp-stft_params](https://github.com/usadellab/prot-fin/tree/85d930fecefa6052a3d5ee49a921d9fa97432d39/experiments/recog_with_fft/results), the F1 Score looks better now, but is still below 0.1. The performance tradeoff is not very good, but the approach still could be useful in future experiments.
